...
def clean_ocr_markdown(input_file, output_file):
    """
    ...
    artifact_patterns = [
        # Internal PDF Links: matches [p16](#page-15-0), [\(p10\)](#page-9-0), ([p17](#page-16-0))
        # Updated to handle escaped parentheses \( and \) within the link text
        r'\[.*p\d+.*\]\(#page-\d+-\d+\)',
        r'\(\[.*p\d+.*\]\(#page-\d+-\d+\)\)',
        ...
        # Updated to include "Figure" as well as "Picture"
    ...
    # Prefix with '> ' to make them blockquotes
    # Pattern looks for lines containing the stats structure
    ...
    # Ensure spaces around the tilde (~)
    # Group 1: First key-value pair
    # Group 2: The tilde with potential existing inconsistent spacing
    # Group 3: Second key-value pair
...
if __name__ == "__main__":
    ...
    OUTPUT_FILENAME = "Mythic-Bastionland-Cleaned_v2.md"
    
    clean_ocr_markdown(INPUT_FILENAME, OUTPUT_FILENAME)