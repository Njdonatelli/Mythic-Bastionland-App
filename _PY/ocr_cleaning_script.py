...
Original file is located at
    https://colab.research.google.com/drive/1SE1eE5qbK9LSeQvd8zgwVVRudelDHfQy
"""
...
def clean_ocr_markdown(input_file, output_file):
    ...
    artifact_patterns = [
        # Internal PDF Links: matches [p16](#page-15-0) or similar variations
        r'\[\??\(?p\d+\??\)?\]\(#page-\d+-\d+\)',
        ...
        # Image Tags: ![](_page_19_Picture_32.jpeg)
        r'!\[\]\(_page_\d+_Picture_\d+\.jpeg\)',

        # OCR Noise Characters: 禁, 张, 弘, 姓
        r'[禁张弘姓]',

        # Isolated Page Numbers: Lines containing only digits (e.g., "12" or " 4 ")
        r'(?m)^\s*\d+\s*$'
    ]
